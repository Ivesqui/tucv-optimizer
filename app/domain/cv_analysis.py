...
def analyze_bullet_quality(bullet: str) -> dict:
    ...
    # --- CAMBIO 1: REGEX UNIVERSAL ---
    # Ahora detecta $ al final, puntos de miles (15.000) y más palabras clave de negocio
    metric_pattern = r"\d+[\d.,]*\s*%|\d+x|[\$€]\s?\d+[\d.,]*|\d+[\d.,]*\s?[\$€]|\d+\s*(usuarios|clientes|ms|segundos|peticiones|servidores|costos|entrega|horas|anuales|proyectos|bugs)"
    ...
    final_score = max(0.0, score / 10)

    if final_score >= 10.0:
        feedback = ["¡Excelente bullet! Este logro cumple con todos los estándares ATS."]
        suggestions = []  # Vaciamos las sugerencias si es un 10
    else:
        feedback = issues if issues else ["¡Buen trabajo! Casi perfecto."]
